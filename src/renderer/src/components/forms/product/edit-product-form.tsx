...
export function UpdateProductForm(props: Props) {
  const { handleEdit, loading, categories, product } = props
  const imageInputRef = useRef<HTMLInputElement>(null)
  const [imageUploadError, setImageUpoadError] = useState('')
  const [preview, setPreview] = useState(product.image)
  const [price, setPrice] = useState<string | undefined>((product.price / 100).toString())
  ...
  useEffect(() => {
    if (product.image) setPreview(import.meta.env.VITE_LOCAL_SERVER_URL + product.image)
  }, [])
