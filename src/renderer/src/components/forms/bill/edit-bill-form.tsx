...
export function UpdateBillForm(props: Props) {
  const { handleEdit, loading, suppliers, bill } = props
  const [cost, setCost] = useState<string | undefined>((bill.cost / 100).toString())
  const [totalPaid, setTotalPaid] = useState<string | undefined>((bill.totalPaid / 100).toString())
  const [dollarRate, setDollarRate] = useState<string | undefined>(
    (bill.dollarRate / 100).toString()
  )
  const [supplierId, setSupplierId] = useState<number | null>(bill.supplierId)
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const updatedBill = await handleEdit({
    ...
    if (updatedBill) {
      form.reset({
        number: updatedBill.number ? updatedBill.number : '',
        isPaid: updatedBill.isPaid,
        notes: updatedBill.notes,
        currency: updatedBill.currency === 'USD' ? '$' : '£',
        date: new Date(updatedBill.date)
      })
      setCost((updatedBill.cost / 100).toString())
      setTotalPaid((updatedBill.totalPaid / 100).toString())
      setDollarRate((updatedBill.dollarRate / 100).toString())
      setSupplierId(updatedBill.supplierId)
    }
  }
